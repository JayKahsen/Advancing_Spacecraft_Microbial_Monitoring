...
# Rarefaction workflow for feature tables.
#
# Purpose:
#   1) Read taxonomic abundance tables from data_tables/original/
#   2) Standardize table format across taxonomic levels
#   3) Optionally rarefy and filter outputs
#
# Note:
#   - This script expects meta + helper objects to already exist via _globalStuff.R.
#   - matrix_names.csv only needs to be regenerated if the project structure changes.

# load file for common project stuff with a non-interactive fallback
script_dir <- tryCatch(
...
# Optional: recreate matrix_names.csv (normally keep as-is once meta exists)
# making_matrix_names='yes'

# Main run mode: format raw taxonomic matrices from existing original inputs
making_raw_matrix='yes'     # keep enabled for normalization/standardization

qPrint(data_sets)
for(data_set in data_sets){print(get(sym(data_set)))}

# Optional diagnostics for selecting minimum sample count
# look_at_counts='yes'
...
if(making_raw_matrix=='yes'){qPrint(making_raw_matrix)
  ...
  for (taxa_levs in taxa_levels){qPrint(taxa_levs)
    ...
    if(taxa_levs=='ASV'){next()
      ...
      file=(paste0('data_tables/original/Merged_',taxa_levs,'_raw_abund.csv'))
    }
    
    df_matrix2<- read.csv(file,check.names = FALSE,row.names = 1) %>% 
    ...
    df_matrix1 <- df_matrix2 %>% 
      ...
      #     str_remove("^.*?_PPR_") %>%                  # remove everything up to and including _PPR_
      #     str_remove("_S\\d{3}.*$")                    # remove _S### and everything after
      # ) %>%
