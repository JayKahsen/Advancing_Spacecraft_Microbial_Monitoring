...
for(plot_set in plot_set_order){qPrint(plot_set)
  ...
  for(lp in Loop_Group_order){
    ...
    for (r in starting_r:ending_r) {
      ...
      for (pg in plot_groups){print(pg)
        ...
        p=ggplot(df_plot, aes(x = x, y = y)) +
          geom_hline(yintercept = 0, linetype = "dashed") +
          geom_vline(xintercept = 0, linetype = "dashed") +
          geom_point(aes(fill = !!sym(filter1_group),size=richness_size
                         #shape = !!sym(shape_group),size = !!sym(shape_group)
          ),
          shape=21
          )+  
   ...
   #   ggsave(paste0(output_plot,p_title,'.png'),patch_plots,width=plot_width+4,height=plot_height+4)
      ...
      ggsave(paste0(output_plot,'Figure_4_',p_title,'_TESTS_ILMAH_',current_date,'.png'),patch_test_plots,width=plot_width,height=plot_height/2)
   ...
   #  ggsave(paste0(output_plot,'Figure_4_',plot_title,'_ILMAH_',current_date,'.png'),patch_plots,width=plot_width+4,height=plot_height+4)
      write.csv(df_plot,paste0(output_plot,'Figure_4_',plot_title,'_ILMAH_',current_date,'.csv'))
      write.csv(df_tests_plot,paste0(output_plot,'Figure_4_',plot_title,'_TESTS_ILMAH_',current_date,'.csv'))
    } # end plotset
  } # end loop/run group
