  ...
  for(lp in Loop_Group_order){
    ...
    for (r in starting_r:ending_r) {
      ...
      for (pg in plot_groups){print(pg)
        ...
        df_plot <- df%>%
          select(sample_name,any_of(grouping_columns),x_nmds_bray,y_nmds_bray,value_bacterial,smp_richness,richness_size) %>% 
          mutate(log10_value_bacterial=log10(value_bacterial+1e-6)) %>% 
          mutate(log10_bacterial_size=case_when(
            log10_value_bacterial>10~'log10',
            log10_value_bacterial>8~'log8',
            log10_value_bacterial>6~'log6',
            log10_value_bacterial>4~'log4',
            log10_value_bacterial>0~'log0',
            TRUE~'less_than_0'
          )) %>% 
          pivot_longer(cols = c('richness_size','log10_bacterial_size'),names_to ='method' ,values_to = 'size') %>% 
        ...
        plot_title
        ################################################################################
        p=ggplot(df_plot %>% filter(method=='log10_bacterial_size'), aes(x = x, y = y)) +
          geom_hline(yintercept = 0, linetype = "dashed") +
          geom_vline(xintercept = 0, linetype = "dashed") +
          geom_point(aes(fill = !!sym(filter1_group),size=size,
                         #shape = !!sym(shape_group),size = !!sym(shape_group)
