...
for(lp in Loop_Group_order){
  ...
  for (r in starting_r:ending_r) {
   ...
   # group_within=c(y_axis_group,y_facet_group)
    ...
    sc = 1 / (min(plot_df$mean_rel_abun[plot_df$mean_rel_abun > 0]) * unique(plot_df$group_counts))
    sc=3
...
#    sc=1e24 # shifts bars on left plot
    ...
    st_plot=ggplot(plot_df, aes(x = mean_rel_abun_log10+group_counts_log10+sc, y = plot_order)) +
      geom_col(aes(fill = !!sym(y_axis_group)),width=.6,position = dodge_pos)+
      geom_jitter(
        data = plot_ind_df,
        aes(y = plot_order,x = rel_abun_log10+group_counts_log10+sc, fill = !!sym(y_axis_group)),
      ...
      # geom_point(
      #   data = plot_ind_df,
      #   aes(y = plot_order,x = rel_abun_log10+group_counts_log10+sc, group = !!sym(y_axis_group)),
      #   position = dodge_pos,
      ...
      # )+
      # geom_errorbarh(
      #   aes(xmin = (xmin_log10+sc), xmax = (xmax_log10+sc), group = !!sym(y_axis_group)),
      #   position = dodge_pos,
     ...
     scale_x_continuous(labels = sc_log10_labels
                        ,breaks = c(-12:12)
                      )
    st_plot
      ...
      st_plot=st_plot+
        geom_text(
          aes(
            ...
            x = ifelse((mean_rel_abun_log10+group_counts_log10) < x_min, x_alt-sc, mean_rel_abun_log10+group_counts_log10+sc),
            color = ifelse((mean_rel_abun_log10+group_counts_log10) < x_min, 'black', 'white'),
    ...
    combined_plot <- st_plot + mn_plot +
      ...
      plot_annotation(
        title = "Non-rarefied data — using average counts to approximate scale",
        caption = "Non-rarefied data — using average counts to approximate scale",
        theme = theme(
    ...
    if(taxa_levs=='Phylum'){
      ...
      ggsave(paste0(output_plot,'Figure_',figure_number,'_',p_title,'_',current_date,'.png'),combined_plot,width=36,height=48)
    }
  ...
  ratio_vector[facet]=n
    ...
    ###
    type_plots <- wrap_plots(rev(plot_list), ncol = 1, heights = rev(ratio_vector)) +
      plot_annotation(caption = "non rarefied data-using average counts to approximate scale ")
    ###
