...
for(lp in Loop_Group_order){
  ...
  for (r in starting_r:ending_r) {
    ...
    mutate(
     ...
     # mutate(label_new=paste(label_new,rel_abun_label)) %>% 
   ...
   #   geom_text(aes(x=Inf,label=adjusted_significance,color=adjusted_significance_color),hjust=1.5)+
    ...
    st_plot=st_plot+
      geom_text(
        aes(
          ...
          x = ifelse(mean_rel_abun/.01 < 90,90, mean_rel_abun/.01),
          #color = ifelse(mean_rel_abun/.01 < 90, 'black', 'white'),
    ...
    df_sample_counts=df_matrix %>% 
      xPlode_sample_name() %>% 
      mutate(!!sym(y_facet_group):=factor(!!sym(y_facet_group), levels = y_facet_group_order))%>%
      mutate(!!sym(y_axis_group):=factor(!!sym(y_axis_group), levels = rev(y_axis_group_order)))%>%
      mutate(!!sym(x_facet_group):=factor(!!sym(x_facet_group), levels = x_facet_group_order))%>%
      select(sample_name,any_of(grouping_columns),sample_reads)
    
    write.csv(df_sample_counts,paste(output_plot,'df_sample_counts.csv'),row.names = FALSE)
    
    library(gghalves)
   
    sample_plot=ggplot(df_sample_counts, aes(x = !!sym(y_axis_group), y = sample_reads)) +
        facet_wrap2(formula(paste('~',x_facet_group))
                    # ,  labeller = labeller(
                    #   .rows = as_labeller(palette_label),
                    #   .cols = as_labeller(palette_label)
                    #)
        ...
        geom_half_violin(aes(fill = !!sym(y_axis_group)),
                         side = "r", # put violin on left
                         alpha = 0.6) +
        geom_half_boxplot(aes(fill = !!sym(y_axis_group)),
                          side = "l",   # boxplot aligned with violin
                          width = 0.2,
                          outlier.shape = NA,  # hides outliers
                          alpha = 0.4) +
        geom_half_point(
          aes(color = !!sym(y_axis_group)),
          side = "l",        # points on left
          shape = 1,
          size = 2,
          alpha = 0.7,
          width = 0.6        # increase this to move points farther from the axis
        )+
      labs(x='',y='',
    ...
    ########################################################################
      ...
      sample_plot <- gPlot(sample_plot)+theme(legend.position = 'none')+labs(y='Sequence Reads')
      sample_plot
    
      sample_plot=sample_plot+  theme(
      ...
      saveRDS(sample_plot,paste0(output_plot,'Figure-',figure_number,'_sample_plot.rds'))
