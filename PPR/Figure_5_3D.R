...
for(plot_set in plot_set_order){qPrint(plot_set)
  ...
  figure_number='5'
  figure_order=c('5','5A','5B','5C')
  parameter_sets <- list(
    set1 = list(
      filter1_group = 'experiment',
      shape_group = 'experiment',
      color_group = 'sampling_device',
  ...
  for(lp in Loop_Group_order){
    ...
    for (r in starting_r:ending_r) {
      ...
      for (pg in plot_group_order){
        ...
        df_3d <- as.data.frame(nmds_bray$points)
      ...
      ################################################################################
      plot_list_richness=plot_list_log10=plot_list =plot_list_anova=plot_list_disp<- list()
      ...
      for (pg in plot_groups){print(pg)
        ...
        df_plot <- df%>%
          #select(sample_name,any_of(grouping_columns),x_nmds_bray,y_nmds_bray,,smp_richness,richness_size) %>% 
          mutate(log10_value=copies_original_dpcr_log10) %>% 
          mutate(log10_size=case_when(
            log10_value>10~'log10',
            log10_value>8~'log8',
            log10_value>6~'log6',
            log10_value>4~'log4',
            log10_value>0~'log0',
          ...
          pivot_longer(cols = c('richness_size','log10_size'),names_to ='method' ,values_to = 'size') %>% 
        ...
        ################################################################################
        print('# plot facets')
        plot_title=paste(p_title,pg,'facets',sep='_')
        plot_title
        ################################################################################
        p=ggplot(df_plot, aes(x = x, y = y)) +
          geom_hline(yintercept = 0, linetype = "dashed") +
          geom_vline(xintercept = 0, linetype = "dashed") +
          geom_point(aes(fill = !!sym(color_group),
          ...
          guides(
            # size = "none",
            color = guide_legend(title='',override.aes = list(linewidth=1.5)), 
            ...
            shape = guide_legend(title='',override.aes = list(size=5)), 
        ...
        p
        gPlot(p)
        plot_facets=gPlot(p)+theme(legend.position = 'bottom')
        plot_facets
        
        # ggsave(paste0(output_plot,plot_title,'.png'),plot_sample_time,width=36,height=10)
        ggsave(paste0(output_plot,'Figure_',figure_number,'_',plot_title,'_PPR_',current_date,'.png'),plot_facets,width=36,height=10)
        ################################################################################ 
        ################################################################################
        print('# plot facet_richness')
        ...
        p=ggplot(df_plot %>% filter(method=='richness_size'), aes(x = x, y = y)) +
          geom_hline(yintercept = 0, linetype = "dashed") +
          geom_vline(xintercept = 0, linetype = "dashed") +
          geom_point(aes(fill = !!sym(color_group),size=size,
                         shape = !!sym(shape_group)
                         #,size = !!sym(shape_group)
          )
          #,shape=21,size=5
          )+  
          #scale_size(range = c(1, 12))+  
        ...
        p
        gPlot(p)
        plot_richness=gPlot(p)+theme(legend.position = 'bottom',legend.box = "vertical")
        plot_richness
        plot_list_richness[[pg]]=plot_richness
        # ggsave(paste0(output_plot,plot_title,'.png'),plot_sample_time,width=36,height=10)
        # ggsave(paste0(output_plot,'Figure_4_',plot_title,'_ILMAH_',current_date,'.png'),plot_sample_time,width=36,height=10)
        ################################################################################ 
        ################################################################################
        print('# plot facet_log10')
        plot_title=paste(p_title,pg,'size_log10',sep='_')
        plot_title
        ################################################################################
        p=ggplot(df_plot %>% filter(method=='log10_size'), aes(x = x, y = y)) +
          geom_hline(yintercept = 0, linetype = "dashed") +
          geom_vline(xintercept = 0, linetype = "dashed") +
          geom_point(aes(fill = !!sym(color_group),size=size,
                         shape = !!sym(shape_group)
          ...
          guides(
            ...
            fill = guide_legend(title='',override.aes = list(shape=22,size=5),order=1,nrow=1), 
            shape = guide_legend(title='',override.aes = list(size=5)), 
            size = guide_legend(title='% Recovery >',order=2,nrow=1), 
          )+
          labs(
            y=paste0("NMDS2; <span style='color:transparent'>_</span> k = ",k_value),
            x=paste0("NMDS1; <span style='color:transparent'>_</span> stress = ",round(stress,2)),
        ...
        p
        gPlot(p)
        plot_log10=gPlot(p)+theme(legend.position = 'bottom')
        plot_log10
        
        plot_list_log10[[pg]]=plot_log10
        # ggsave(paste0(output_plot,plot_title,'.png'),plot_sample_time,width=36,height=10)
        #ggsave(paste0(output_plot,'Figure_4_',plot_title,'_ILMAH_',current_date,'.png'),plot_sample_time,width=36,height=10)
        ################################################################################ 
      ...
      combined_facet_list <- Map(function(top,bottom) {
        wrap_elements(top) / wrap_elements(bottom) +
          plot_layout(heights = c(1,1))
      },plot_list_richness, plot_list_log10)
      ...
      patch_combined_plots
      
      patch_test_plots <- wrap_plots(combined_test_list)
      print(patch_test_plots)
      
      patch_facet_plots <- wrap_plots(combined_facet_list)
      patch_facet_plots
      ...
      ggsave(paste0(output_plot,'Figure_',figure_number,'_',p_title,'_combined_',current_date,'.png'),patch_combined_plots,width=plot_width+4,height=plot_height+8)
      ggsave(paste0(output_plot,'Figure_',figure_number,'_',p_title,'_tests_',current_date,'.png'),patch_test_plots,width=plot_width+4,height=plot_height+8)
      ggsave(paste0(output_plot,'Figure_',figure_number,'_',p_title,'_facets_',current_date,'.png'),patch_facet_plots,width=plot_width+4,height=plot_height+8)
      
      write.csv(df_plot,paste0(output_plot,'Figure_',figure_number,'_',plot_title,'_PPR_',current_date,'.csv'),row.names=FALSE)
      write.csv(df_tests,paste0(output_plot,'Figure_',figure_number,'_',plot_title,'_TESTS_PPR_',current_date,'.csv'),row.names=FALSE)
...
names(df_plot)
df_plot3D=df_plot %>% 
  select(x,y,z_nmds_bray,sample_name,stress,sampling_device) %>%
