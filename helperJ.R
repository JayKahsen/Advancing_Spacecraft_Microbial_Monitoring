...
################################################################################
# Replacement block (landmark: qWrite <- function(df, name = NULL, folder = NULL) { ... })

qWrite <- function(df, name = NULL, folder = NULL) {
  # Example: qWrite(loadings_long_df, folder = "both")
  
  if (is.null(name)) {
    name <- deparse(substitute(df))
  }
  
  # Default controlled by outside variable (if it exists)
  write_both <- FALSE
  if (exists("qWrite_write_both", envir = parent.frame())) {
    write_both <- get("qWrite_write_both", envir = parent.frame())
  }
  
  # Special case: folder explicitly requests both outputs
  if (!is.null(folder) && is.character(folder) && length(folder) == 1 && folder == "both") {
    output_data <- get("output_data", envir = parent.frame())
    output_plot <- get("output_plot", envir = parent.frame())
    
    filename_data <- paste0(output_data, name, ".csv")
    filename_plot <- paste0(output_plot, name, ".csv")
    
    qPrint(filename_data)
    write.csv(df, filename_data, row.names = FALSE)
    
    qPrint(filename_plot)
    write.csv(df, filename_plot, row.names = FALSE)
    
    return(invisible(list(output_data = filename_data, output_plot = filename_plot)))
  }
  
  # Default behavior when folder is NULL
  if (is.null(folder)) {
    output_data <- get("output_data", envir = parent.frame())
    
    if (isTRUE(write_both)) {
      output_plot <- get("output_plot", envir = parent.frame())
      ...
      filename <- paste0(output_data, name, ".csv")
      qPrint(filename)
  ...
  # If folder is explicitly provided (and not "both"), respect it (single write)
...
qSave <- function(x,
                  ...
                  plot_height = NULL,
                  show_plot = TRUE) {
  # Example: qSave("hambone", ext = c("png","pdf","svg","rds"))
  ...
  for (ext_i in ext) {
    ...
    # RDS: save the ggplot object directly
    if (ext_i == "rds") {
...
summary_brackets <- function(
  ...
  # df=df_plot1; axis_column="treatment"; dodge_column=NULL; value_column="value";
  # facet_columns="location"; testing_col="treatment"; test_type=NULL;
  # adjust_method="BH"; dodge_width=0.8; width=0.8; log=TRUE; space_adj=1;
  # bracket_space_adj=1; tip_adj=1; run_tests=TRUE; clear_ns_ANOVA=FALSE; clear_ns_message=FALSE;
  # sample_id="sample_id"; adj_y_max_columns=NULL;
  ...
  df_combs <- df %>%
    ...
    summarise(
      ...
      groups = list(unique(stats::na.omit(as.character(.data[[testing_col]])))),
      .groups = "drop"
    ) %>%
    mutate(
      pairs = purrr::map(groups, ~ {
        ...
        utils::combn(sort(.x), 2, simplify = FALSE) %>%
          purrr::map(~ {
            g1 <- .x[1]
            g2 <- .x[2]
            tibble(
...
set_output <- function(subfolder = NULL) {
  # Example: set_output(script_title)
