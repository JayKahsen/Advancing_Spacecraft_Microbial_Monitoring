...
for(lp in Loop_Group_order){
  ...
  for (r in starting_r:ending_r) {
    ...
    ################################################################################
    # ANCOM_name=paste('output_data/ANCOM_group_results_',figure_number,'.csv')
    # qLoad(ANCOM_name)
    # if(!exists(ANCOM_name)){
    # ANCOM_group_results <- ANCOM_BC_Test(df = df_matrix2, testing_group = y_axis_group, category_group = test_across,tax_level = 'Genus')
    # write.csv(ANCOM_group_results,ANCOM_name)}
    ...
    st_plot=ggplot(plot_df, aes(x = mean_rel_abun_log10+sc, y = plot_order)) +
    ...
    mn_plot=ggplot(mn_plot_df, aes(x = normalized_rel_abun, y = plot_order, fill = !!sym(y_axis_group))) +
      ...
      geom_text(aes(x=Inf,label=significance,color=significance),hjust=1.5)+
    ...
    st_plot=st_plot+
      geom_text(
        aes(
          ...
          x = ifelse((mean_rel_abun_log10+sc) < 1, 1, mean_rel_abun_log10+sc),
          color = ifelse((mean_rel_abun_log10+sc) < 1, 'black', 'white'),
    ...
    st_plot=gPlot(st_plot)+labs(x='Relative Abundance')
    
    mn_plot=gPlot(mn_plot)+labs(x='Normalized Relative Abundance')
