...
df_tests=df_tests1 %>% 
  ...
  mutate(significance=mann_log_sig) %>% 
  mutate(p.value=mann_log_val) 
################################################################################
################################################################################
library(ggplot2)
library(ggsignif)
...
ggsave(paste0(output_plot,plot_title,current_date,'_A.pdf'),plotA,width=10,height=8,dpi=dpi)
saveRDS(plotA,paste0(output_plot, plot_title, current_date, "_A.rds")
)
