...
################################################################################
script_title <- 'Figure 6 NMDS Clipper'
    ...
    for (r in starting_r:ending_r) {
      ...
      for (pg in plot_group_order){
        ...
        run_permdisp <- function(dist_matrix, grouping_var) {
          ...
          average_dist_median <- paste0(names(group_means), "=", group_means, collapse = " ")
      ...
      for (pg in plot_groups){print(pg)
        ...
        ################################################################################
        print('450 # Annotations as NMDS begin')
        ################################################################################
        col_order=c('test','testing','formula','Term','average_dist_median','R2','Df','F_value','p_value')
        ...
        p
        facets=gPlot(p)+theme(legend.position = 'bottom')
        facets
        # ggsave(paste0(output_plot,plot_title,'.png'),plot_sample_time,width=36,height=10)
        ggsave(paste0(output_plot,plot_title,current_date,'.png'),facets,width=36,height=10)
        ################################################################################ 
        ################################################################################
        print('# plot sample_time_richness')
        plot_title=paste(p_title,pg,'facets_richness',sep='_')
        plot_title
        ################################################################################
        p=ggplot(df_plot, aes(x = x, y = y)) +
          geom_hline(yintercept = 0, linetype = "dashed") +
        ...
        plot_sample_time
        # ggsave(paste0(output_plot,plot_title,'.png'),plot_sample_time,width=36,height=10)
        ggsave(paste0(output_plot,plot_title,current_date,'.png'),plot_sample_time,width=36,height=10)
        ################################################################################ 
      ...
      ggsave(paste0(output_plot,p_title,'_TESTS_',current_date,'.png'),patch_test_plots,width=plot_width,height=plot_height/2)
      
   #  ggsave(paste0(output_plot,'Figure_4_',plot_title,'_ILMAH_',current_date,'.png'),patch_plots,width=plot_width+4,height=plot_height+4)
      write.csv(df_plot,paste0(output_plot,plot_title,current_date,'.csv'),row.names = FALSE)
      write.csv(df_tests_plot,paste0(output_plot,plot_title,'_TESTS_',current_date,'.csv'),row.names = FALSE)
    } # end plotset
  } # end loop/run group
